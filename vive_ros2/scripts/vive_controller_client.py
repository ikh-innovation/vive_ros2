"""
Defines Vive Controller server. This script should run as is.

Example usage:
python vive_controller_client.py --debug True
...
from vive_server.models import ViveDynamicControllerMessage
import json
...
class ViveControllerClient:
    """
    Defines a vive controller client that constantly polls message from (HOST, PORT)
    and update its self.latest_controller_message public variable

    Other interacting script can initialize this ViveController Client as a sub-process and access its
    latest_controller_message for controller data.

    multiple vive controllers can be used at the same time by initializing multiple clients with different `controller_name`
    """
    ...
    def __init__(self, host: str, port: int, controller_name: str,
        ...
        Args:
            ...
            controller_name: Controller name (Ex: controller_1)
        ...
        self.latest_controller_message: Optional[ViveDynamicControllerMessage] = None
        ...
        self.count = 0
        self.logger = logging.getLogger(f"Vive Controller Client [{self.controller_name}]")
        self.logger.info("Controller Initialized")

    def update(self):
        """
        This client will send to the server the name of the controller it is requesting

        It will receive that controller's information.

        Updates the self.latest_vive_controller_message field

        Record  self.latest_vive_controller_message if needed

        Returns:
            None
        """
        self.logger.info(f"Start Subscribing to [{self.host}:{self.port}] "
                         f"for [{self.controller_name}] Vive Controller Updates")
        while True:
            try:
                _ = self.socket.sendto(self.controller_name.encode(), (self.host, self.port))
                ...
                if status:
                    self.update_latest_controller_message(parsed_message=parsed_message)
                    if self.should_record:
                        if self.count % 10 == 0:
                            self.output_file.write(f'{self.latest_controller_message.x},'
                                                   f'{self.latest_controller_message.y},'
                                                   f'{self.latest_controller_message.z},'
                                                   f'{self.latest_controller_message.roll},'
                                                   f'{self.latest_controller_message.pitch},'
                                                   f'{self.latest_controller_message.yaw}\n')
                    self.count += 1
                else:
    ...
    def run_threaded(self, queue, kill):
        ...
        while not kill.is_set():
            try:
                ...
                if status:
                    self.update_latest_controller_message(parsed_message=parsed_message)
                    queue.put(self.latest_controller_message)
                    if self.should_record:
                        if self.count % 10 == 0:
                            self.output_file.write(f'{self.latest_controller_message.x},'
                                                   f'{self.latest_controller_message.y},'
    ...
    def update_latest_controller_message(self, parsed_message):
        """
        Given Vive Controller message in JSON format, load json into dictionary format,
        parse the controller message using PyDantic

        Assign self.latest_vive_controller_message as the parsed result

        Args:
            parsed_message: controller message in json format
        ...
        try:
            ...
            vive_controller_message = ViveDynamicControllerMessage.parse_obj(d)
            if vive_controller_message.device_name == self.controller_name:
                self.latest_controller_message = vive_controller_message
            self.logger.debug(self.latest_controller_message)
        except Exception as e:
...
if __name__ == "__main__":
    ...
    client = ViveControllerClient(host=HOST, port=PORT, controller_name="controller_1", should_record=args.collect)
    client.update()
